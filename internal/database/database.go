...
func (s *service) CreateTrace(tr TraceRequest) error {
	...
	var tags interface{}
	if len(tr.Tags) > 0 {
		tags = pq.Array(tr.Tags)
	}

	_, err = s.db.ExecContext(ctx, query, tr.ID, tr.Name, metadata, tags, tr.UserID, tr.SessionID, tr.StartTime, tr.EndTime)
	if err != nil {
...
func (s *service) CreateGeneration(gr GenerationRequest) error {
	...
	query := `INSERT INTO generations (id, trace_id, name, input, output, model, prompt_tokens, completion_tokens, total_tokens, metadata, start_time, end_time)
